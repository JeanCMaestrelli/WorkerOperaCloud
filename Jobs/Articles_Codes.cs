...
{
    public class Articles_Codes : IServiceJobs
    {
        private readonly ILogger<Articles_Codes> _logger;
        ...
        public Articles_Codes(ILogger<Articles_Codes> logger, DapperContext context, IConfiguration config, Job_Exec Job_Exec)
        {
            _logger = logger;
            _context = context;
            _Job_Exec = Job_Exec;
        ...
        public void IntegrarJobAsync(string filePath, string id_job, string TIPO_AGENDAMENTO)
        {
            ...
            var List = _Job_Exec.Get_List<Mdl_Articles_Codes>(filePath,LoggBuilder);
                    ...
                    foreach (var trial in List)
                    {
                        try
                        {
                            var _where = $"RESORT = '{trial.RESORT}' and " +
                                $"ARTICLE_ID = '{trial.ARTICLE_ID}' and " +
                                $"ARTICLE_CODE = '{trial.ARTICLE_CODE}' and " +
                                $"TRX_CODE = '{trial.TRX_CODE}'";

                            var query = $"select RNA_UPDATEDATE from {TIPO_AGENDAMENTO} where {_where}";

                            var result = command.Connection.Query<Mdl_Articles_Codes>(query);
        ...
        private OracleCommand MountParametros(OracleCommand parametros, Mdl_Articles_Codes Trxs_Codes)
        {
            parametros.Parameters.Add("RESORT", Trxs_Codes.RESORT);
            parametros.Parameters.Add("ARTICLE_ID", Trxs_Codes.ARTICLE_ID);
            parametros.Parameters.Add("ARTICLE_CODE", Trxs_Codes.ARTICLE_CODE);
            parametros.Parameters.Add("TRX_CODE", Trxs_Codes.TRX_CODE);
            parametros.Parameters.Add("DESCRIPTION", Trxs_Codes.DESCRIPTION);
            parametros.Parameters.Add("DEFAULT_PRICE", Trxs_Codes.DEFAULT_PRICE.Replace(".",","));
            ...
            parametros.Parameters.Add("POSTIT_COLOUR", Trxs_Codes.POSTIT_COLOUR);
            parametros.Parameters.Add("UPC_CODE", Trxs_Codes.UPC_CODE);
