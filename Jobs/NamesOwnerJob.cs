...
{
    public class NamesOwnerJob : IServiceJobs
    {
        private readonly ILogger<NamesOwnerJob> _logger;
        ...
        public NamesOwnerJob(ILogger<NamesOwnerJob> logger, DapperContext context, IConfiguration config, Job_Exec Job_Exec)
        {
            _logger = logger;
            _context = context;
            _Job_Exec = Job_Exec;
        ...
        public void IntegrarJobAsync(string filePath, string id_job, string TIPO_AGENDAMENTO)
        {
            ...
            var List = _Job_Exec.Get_List<Mdl_NamesOwner>(filePath, LoggBuilder);
            ...
            if (List.Count > 0)
            {
                try
                {
                    ...
                    foreach (var trial in List)
                    {
                        ...
                        try
                        {
                            var _where = $"NAME_ID = '{trial.NAME_ID}' and " +
                                $"RESORT = '{trial.RESORT}' and " +
                                $"USER_ID = '{trial.USER_ID}'";

                            var query = $"select RNA_UPDATEDATE from {TIPO_AGENDAMENTO} \nwhere {_where}";

                            var result = command.Connection.Query<Mdl_NamesOwner>(query);
        ...
        private OracleCommand MountParametros(OracleCommand parametros, Mdl_NamesOwner Trxs_Codes)
        {
            parametros.Parameters.Add("NAME_ID", Trxs_Codes.NAME_ID);
            parametros.Parameters.Add("RESORT", Trxs_Codes.RESORT);
            parametros.Parameters.Add("USER_ID", Trxs_Codes.USER_ID);
            parametros.Parameters.Add("PRIMARY_YN", Trxs_Codes.PRIMARY_YN);
            parametros.Parameters.Add("RELATIONSHIP", Trxs_Codes.RELATIONSHIP);
            parametros.Parameters.Add("TO_TYPE", Trxs_Codes.TO_TYPE);
