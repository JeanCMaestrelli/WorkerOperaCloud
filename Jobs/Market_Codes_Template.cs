...
{
    public class Market_Codes_Template : IServiceJobs
    {
        private readonly ILogger<Market_Codes_Template> _logger;
        ...
        public Market_Codes_Template(ILogger<Market_Codes_Template> logger, DapperContext context, IConfiguration config, Job_Exec Job_Exec)
        {
            _logger = logger;
            _context = context;
            _Job_Exec = Job_Exec;
        ...
        public void IntegrarJobAsync(string filePath, string id_job, string TIPO_AGENDAMENTO)
        {
            ...
            var List = _Job_Exec.Get_List<Mdl_Market_Codes_Template>(filePath, LoggBuilder);
            ...
            if (List.Count > 0)
            {
                try
                {
                    ...
                    foreach (var trial in List)
                    {
                        try
                        {
                            var _where = $"MARKET_CODE = '{trial.MARKET_CODE}' and \n" +
                                $"PARENT_MARKET_CODE = '{trial.PARENT_MARKET_CODE}' and \n" +
                                $"SELL_SEQUENCE = '{trial.SELL_SEQUENCE}' and \n" +
                                $"DISPLAY_COLOR = '{trial.DISPLAY_COLOR}'";

                            var query = $"select RNA_UPDATEDATE from {TIPO_AGENDAMENTO} \nwhere {_where}";

                            var result = command.Connection.Query<Mdl_Market_Codes_Template>(query);
        ...
        private OracleCommand MountParametros(OracleCommand parametros, Mdl_Market_Codes_Template Trxs_Codes)
        {
            parametros.Parameters.Add("MARKET_CODE", Trxs_Codes.MARKET_CODE);
            parametros.Parameters.Add("PARENT_MARKET_CODE", Trxs_Codes.PARENT_MARKET_CODE);
            ...
            parametros.Parameters.Add("SC_ORDERBY", Trxs_Codes.SC_ORDERBY);
            ...
            parametros.Parameters.Add("PRINT_GROUP", Trxs_Codes.PRINT_GROUP);
