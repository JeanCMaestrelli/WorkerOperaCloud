...
{
    public class Resort_Origins_Of_Booking : IServiceJobs
    {
        private readonly ILogger<Resort_Origins_Of_Booking> _logger;
        ...
        public Resort_Origins_Of_Booking(ILogger<Resort_Origins_Of_Booking> logger, DapperContext context, IConfiguration config, Job_Exec Job_Exec)
        {
            _logger = logger;
            _context = context;
            _Job_Exec = Job_Exec;
        ...
        public void IntegrarJobAsync(string filePath, string id_job, string TIPO_AGENDAMENTO)
        {
            ...
            var List = _Job_Exec.Get_List<Mdl_Resort_Origins_Of_Booking>(filePath, LoggBuilder);
                    ...
                    foreach (var trial in List)
                    {
                        ...
                        try
                        {
                            var _where = $"RESORT = '{trial.RESORT}' and " +
                                $"SOURCE_CODE = '{trial.SOURCE_CODE}' and " +
                                $"SELL_SEQUENCE = '{trial.SELL_SEQUENCE}'";

                            var query = $"select RNA_UPDATEDATE from {TIPO_AGENDAMENTO} \n where {_where}";

                            var result = command.Connection.Query<Mdl_Resort_Origins_Of_Booking>(query);
        ...
        private OracleCommand MountParametros(OracleCommand parametros, Mdl_Resort_Origins_Of_Booking Trxs_Codes)
        {
            parametros.Parameters.Add("RESORT", Trxs_Codes.RESORT);
            parametros.Parameters.Add("SOURCE_CODE", Trxs_Codes.SOURCE_CODE);
            parametros.Parameters.Add("PARENT_SOURCE_CODE", Trxs_Codes.PARENT_SOURCE_CODE);
            ...
            parametros.Parameters.Add("INTERNET_SALES_YN", Trxs_Codes.INTERNET_SALES_YN);
