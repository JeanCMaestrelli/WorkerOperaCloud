...
{
    public class Namesref : IServiceJobs
    {
        private readonly ILogger<Namesref> _logger;
        ...
        public Namesref(ILogger<Namesref> logger, DapperContext context, IConfiguration config, Job_Exec Job_Exec)
        {
            _logger = logger;
            _context = context;
            _Job_Exec = Job_Exec;
        ...
        public void IntegrarJobAsync(string filePath, string id_job, string TIPO_AGENDAMENTO)
        {
            ...
            var List = _Job_Exec.Get_List<Mdl_Namesref>(filePath, LoggBuilder);
            ...
            if (List.Count > 0)
            {
                try
                {
                    ...
                    foreach (var trial in List)
                    {
                        ...
                        try
                        {
                            var _where = $"NAME_XREF_ID = '{trial.NAME_XREF_ID}' and " +
                                $"RESORT = '{trial.RESORT}'";

                            var query = $"select RNA_UPDATEDATE from {TIPO_AGENDAMENTO} \nwhere {_where}";

                            var result = command.Connection.Query<Mdl_Namesref>(query);
        ...
        private OracleCommand MountParametros(OracleCommand parametros, Mdl_Namesref Trxs_Codes)
        {
            parametros.Parameters.Add("NAME_XREF_ID", Trxs_Codes.NAME_XREF_ID);
            parametros.Parameters.Add("NAME_ID", Trxs_Codes.NAME_ID);
            parametros.Parameters.Add("RELATIONSHIP_TYPE", Trxs_Codes.RELATIONSHIP_TYPE);
            parametros.Parameters.Add("RELATIONSHIP_DESC", Trxs_Codes.RELATIONSHIP_DESC);
            parametros.Parameters.Add("RELATION_TO_NAME_ID", Trxs_Codes.RELATION_TO_NAME_ID);
            parametros.Parameters.Add("PRIMARY_YN", Trxs_Codes.PRIMARY_YN);
            parametros.Parameters.Add("RELATIONSHIP_ROLE", Trxs_Codes.RELATIONSHIP_ROLE);
