...
{
    public class Priorities : IServiceJobs
    {
        private readonly ILogger<Priorities> _logger;
        ...
        public Priorities(ILogger<Priorities> logger, DapperContext context, IConfiguration config, Job_Exec Job_Exec)
        {
            _logger = logger;
            _context = context;
            _Job_Exec = Job_Exec;
        ...
        public void IntegrarJobAsync(string filePath, string id_job, string TIPO_AGENDAMENTO)
        {
            ...
            var List = _Job_Exec.Get_List<Mdl_Priorities>(filePath, LoggBuilder);
                    ...
                    foreach (var trial in List)
                    {
                        ...
                        try
                        {
                            var _where = $"RESORT = '{trial.RESORT}' and " +
                                $"PRIORITY_CODE = '{trial.PRIORITY_CODE}' and " +
                                $"PRIORITY_SEQ_NUMBER = '{trial.PRIORITY_SEQ_NUMBER}' and " +
                                $"DESCRIPTION = '{trial.DESCRIPTION}'";

                            var query = $"select RNA_UPDATEDATE from {TIPO_AGENDAMENTO} \n where {_where}";

                            var result = command.Connection.Query<Mdl_Priorities>(query);
        ...
        private OracleCommand MountParametros(OracleCommand parametros, Mdl_Priorities Trxs_Codes)
        {
            ...
            parametros.Parameters.Add("PRIORITY_SEQ_NUMBER", Trxs_Codes.PRIORITY_SEQ_NUMBER);
            parametros.Parameters.Add("COLOR_CODE", Trxs_Codes.COLOR_CODE);
            parametros.Parameters.Add("COLOR_CODE_DESCRIPTION", Trxs_Codes.COLOR_CODE_DESCRIPTION);
