...
{
    public class Room_Classes_Template : IServiceJobs
    {
        private readonly ILogger<Room_Classes_Template> _logger;
        ...
        public Room_Classes_Template(ILogger<Room_Classes_Template> logger, DapperContext context, IConfiguration config, Job_Exec Job_Exec)
        {
            _logger = logger;
            _context = context;
            _Job_Exec = Job_Exec;
        ...
        public void IntegrarJobAsync(string filePath, string id_job, string TIPO_AGENDAMENTO)
        {
            ...
            var List = _Job_Exec.Get_List<Mdl_Room_Classes_Template>(filePath, LoggBuilder);
            ...
            if (List.Count > 0)
            {
                try
                {
                    ...
                    foreach (var trial in List)
                    {
                        ...
                        try
                        {
                            var _where = $"ROOM_CLASS = '{trial.ROOM_CLASS}'";

                            var query = $"select RNA_UPDATEDATE from {TIPO_AGENDAMENTO} \n where {_where}";

                            var result = command.Connection.Query<Mdl_Room_Classes_Template>(query);
        ...
        private OracleCommand MountParametros(OracleCommand parametros, Mdl_Room_Classes_Template Trxs_Codes)
        {
            parametros.Parameters.Add("ROOM_CLASS", Trxs_Codes.ROOM_CLASS);
            parametros.Parameters.Add("ROOM_CLASS_RANK", Trxs_Codes.ROOM_CLASS_RANK);
