...
{
    public class Resort_Rate_CategoryJob : IServiceJobs
    {
        private readonly ILogger<Resort_Rate_CategoryJob> _logger;
        ...
        public Resort_Rate_CategoryJob(ILogger<Resort_Rate_CategoryJob> logger, DapperContext context, IConfiguration config, Job_Exec Job_Exec)
        {
            _logger = logger;
            _context = context;
            _Job_Exec = Job_Exec;
        ...
        public void IntegrarJobAsync(string filePath, string id_job, string TIPO_AGENDAMENTO)
        {
            ...
            var List = _Job_Exec.Get_List<Mdl_Resort_Rate_Category>(filePath, LoggBuilder);
                    ...
                    foreach (var trial in List)
                    {
                        ...
                        try
                        {
                            var _where = $"RESORT = '{trial.RESORT}' and " +
                                ...
                                $"RATE_CLASS = '{trial.RATE_CLASS}'";

                            var query = $"select RNA_UPDATEDATE from {TIPO_AGENDAMENTO} \n where {_where}";

                            var result = command.Connection.Query<Mdl_Resort_Rate_Category>(query);
        ...
        private OracleCommand MountParametros(OracleCommand parametros, Mdl_Resort_Rate_Category Trxs_Codes)
        {
            parametros.Parameters.Add("RESORT", Trxs_Codes.RESORT);
            parametros.Parameters.Add("RATE_CATEGORY", Trxs_Codes.RATE_CATEGORY);
            parametros.Parameters.Add("DESCR", Trxs_Codes.DESCR);
            ...
            parametros.Parameters.Add("RATE_TIER", Trxs_Codes.RATE_TIER);
            ...
            parametros.Parameters.Add("DISPLAY_DEFAULT_YN", Trxs_Codes.DISPLAY_DEFAULT_YN);
            parametros.Parameters.Add("DISPLAY_SET", Trxs_Codes.DISPLAY_SET);
            parametros.Parameters.Add("EXPORT_BUCKET_CODE", Trxs_Codes.EXPORT_BUCKET_CODE);
