...
{
    public class GemEventRevenueJob : IServiceJobs
    {
        private readonly ILogger<GemEventRevenueJob> _logger;
        ...
        public GemEventRevenueJob(ILogger<GemEventRevenueJob> logger, DapperContext context, IConfiguration config, Job_Exec Job_Exec)
        {
            _logger = logger;
            _context = context;
            _Job_Exec = Job_Exec;
        ...
        public void IntegrarJobAsync(string filePath, string id_job, string TIPO_AGENDAMENTO)
        {
            ...
            var List = _Job_Exec.Get_List<Mdl_GemEventRevenue>(filePath, LoggBuilder);
            ...
            if (List.Count > 0)
            {
                try
                {
                    ...
                    foreach (var trial in List)
                    {
                        try
                        {
                            var _where = $"EVENT_ID = '{trial.EVENT_ID}' and \n" +
                                $"RESORT = '{trial.RESORT}' and \n" +
                                $"BOOK_ID = '{trial.BOOK_ID}' and \n" +
                                $"REV_TYPE = '{trial.REV_TYPE}' and \n" +
                                $"REV_GROUP = '{trial.REV_GROUP}'";

                            var query = $"select RNA_UPDATEDATE from {TIPO_AGENDAMENTO} \nwhere {_where}";

                            var result = command.Connection.Query<Mdl_GemEventRevenue>(query);
        ...
        private OracleCommand MountParametros(OracleCommand parametros, Mdl_GemEventRevenue Trxs_Codes)
        {
            ...
            parametros.Parameters.Add("REV_TYPE", Trxs_Codes.REV_TYPE);
            parametros.Parameters.Add("REV_GROUP", Trxs_Codes.REV_GROUP);
            parametros.Parameters.Add("ORDER_BY", Trxs_Codes.ORDER_BY);
            parametros.Parameters.Add("FORECAST_REVENUE", Trxs_Codes.FORECAST_REVENUE);
            parametros.Parameters.Add("EXPECTED_REVENUE", Trxs_Codes.EXPECTED_REVENUE);
            parametros.Parameters.Add("GUARANTEED_REVENUE", Trxs_Codes.GUARANTEED_REVENUE);
            parametros.Parameters.Add("ACTUAL_REVENUE", Trxs_Codes.ACTUAL_REVENUE);
            parametros.Parameters.Add("BILLED_REVENUE", Trxs_Codes.BILLED_REVENUE);
            parametros.Parameters.Add("IGNORE_FORECAST_YN", Trxs_Codes.IGNORE_FORECAST_YN);
            parametros.Parameters.Add("FLAT_YN", Trxs_Codes.FLAT_YN);
            parametros.Parameters.Add("CUSTOM_YN", Trxs_Codes.CUSTOM_YN);
            parametros.Parameters.Add("EXPECTED_COST", Trxs_Codes.EXPECTED_COST);
            parametros.Parameters.Add("GUARANTEED_COST", Trxs_Codes.GUARANTEED_COST);
            parametros.Parameters.Add("ACTUAL_COST", Trxs_Codes.ACTUAL_COST);
            parametros.Parameters.Add("BILLED_COST", Trxs_Codes.BILLED_COST);
            ...
            parametros.Parameters.Add("PKG_REVENUE_YN", Trxs_Codes.PKG_REVENUE_YN);
            parametros.Parameters.Add("FORECAST_EDITED_YN", Trxs_Codes.FORECAST_EDITED_YN);
