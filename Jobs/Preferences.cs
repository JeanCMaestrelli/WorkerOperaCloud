...
{
    public class Preferences : IServiceJobs
    {
        private readonly ILogger<Preferences> _logger;
        ...
        public Preferences(ILogger<Preferences> logger, DapperContext context, IConfiguration config, Job_Exec Job_Exec)
        {
            _logger = logger;
            _context = context;
            _Job_Exec = Job_Exec;
        ...
        public void IntegrarJobAsync(string filePath, string id_job, string TIPO_AGENDAMENTO)
        {
            ...
            var List = _Job_Exec.Get_List<Mdl_Preferences>(filePath, LoggBuilder);
            ...
            if (List.Count > 0)
            {
                try
                {
                    ...
                    foreach (var trial in List)
                    {
                        ...
                        try
                        {
                            var _where = $"PREFERENCE_SEQ_ID = '{trial.PREFERENCE_SEQ_ID}'";

                            var query = $"select RNA_UPDATEDATE from {TIPO_AGENDAMENTO} \n where {_where}";

                            var result = command.Connection.Query<Mdl_Preferences>(query);
        ...
        private OracleCommand MountParametros(OracleCommand parametros, Mdl_Preferences Trxs_Codes)
        {
            parametros.Parameters.Add("PREFERENCE", Trxs_Codes.PREFERENCE);
            parametros.Parameters.Add("PREFERENCE_TYPE", Trxs_Codes.PREFERENCE_TYPE);
            ...
            parametros.Parameters.Add("CRS_PREFERENCE_YN", Trxs_Codes.CRS_PREFERENCE_YN);
            ...
            parametros.Parameters.Add("PREFERENCE_SEQ_ID", Trxs_Codes.PREFERENCE_SEQ_ID);
            ...
            parametros.Parameters.Add("SEND_DELETE_REQUEST_YN", Trxs_Codes.SEND_DELETE_REQUEST_YN);
            parametros.Parameters.Add("CORPORATE_YN", Trxs_Codes.CORPORATE_YN);
            parametros.Parameters.Add("OWS_ALLOWED_YN", Trxs_Codes.OWS_ALLOWED_YN);
            parametros.Parameters.Add("AMENITY_YN", Trxs_Codes.AMENITY_YN);
            parametros.Parameters.Add("ROOM_ASSIGNMENT_VALUE", Trxs_Codes.ROOM_ASSIGNMENT_VALUE);
            parametros.Parameters.Add("PREFERENCE_ATTRIBUTE", Trxs_Codes.PREFERENCE_ATTRIBUTE);
            parametros.Parameters.Add("HOUSEKEEPING_YN", Trxs_Codes.HOUSEKEEPING_YN);
            parametros.Parameters.Add("PREFERENCE_SUB_TYPE", Trxs_Codes.PREFERENCE_SUB_TYPE);
