...
{
    public class Name_CommissionJob : IServiceJobs
    {
        private readonly ILogger<Name_CommissionJob> _logger;
        ...
        public Name_CommissionJob(ILogger<Name_CommissionJob> logger, DapperContext context, IConfiguration config, Job_Exec Job_Exec)
        {
            _logger = logger;
            _context = context;
            _Job_Exec = Job_Exec;
        ...
        public void IntegrarJobAsync(string filePath, string id_job, string TIPO_AGENDAMENTO)
        {
            ...
            var List = _Job_Exec.Get_List<Mdl_Name_Commission>(filePath, LoggBuilder);
                    ...
                    foreach (var trial in List)
                    {
                        ...
                        try
                        {
                            var _where = $"NAME_ID = '{trial.NAME_ID}' and " +
                                ...
                                $"COMMISSION_CODE = '{trial.COMMISSION_CODE}' and " +
                                $"ACCOUNT_ID = '{trial.ACCOUNT_ID}'";

                            var query = $"select RNA_UPDATEDATE from {TIPO_AGENDAMENTO} \nwhere {_where}";

                            var result = command.Connection.Query<Mdl_Name_Commission>(query);
        ...
        private OracleCommand MountParametros(OracleCommand parametros, Mdl_Name_Commission Trxs_Codes)
        {
            ...
            parametros.Parameters.Add("ACCOUNT_ID", Trxs_Codes.ACCOUNT_ID);
