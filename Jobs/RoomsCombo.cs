...
{
    public class RoomsCombo : IServiceJobs
    {
        private readonly ILogger<RoomsCombo> _logger;
        ...
        public RoomsCombo(ILogger<RoomsCombo> logger, DapperContext context, IConfiguration config, Job_Exec Job_Exec)
        {
            _logger = logger;
            _context = context;
            _Job_Exec = Job_Exec;
        ...
        public void IntegrarJobAsync(string filePath, string id_job, string TIPO_AGENDAMENTO)
        {
            ...
            var List = _Job_Exec.Get_List<Mdl_RoomsCombo>(filePath, LoggBuilder);
                    ...
                    foreach (var trial in List)
                    {
                        ...
                        try
                        {
                            var _where = $"RESORT = '{trial.RESORT}' and " +
                                $"COMBO_ROOM = '{trial.COMBO_ROOM}' and " +
                                $"COMBO_ELEMENT = '{trial.COMBO_ELEMENT}'";

                            var query = $"select RNA_UPDATEDATE from {TIPO_AGENDAMENTO} \n where {_where}";

                            var result = command.Connection.Query<Mdl_RoomsCombo>(query);
        ...
        private OracleCommand MountParametros(OracleCommand parametros, Mdl_RoomsCombo Trxs_Codes)
        {
            parametros.Parameters.Add("RESORT", Trxs_Codes.RESORT);
            parametros.Parameters.Add("COMBO_ROOM", Trxs_Codes.COMBO_ROOM);
            parametros.Parameters.Add("COMBO_ELEMENT", Trxs_Codes.COMBO_ELEMENT);
            parametros.Parameters.Add("PERCENT_UTILIZED", Trxs_Codes.PERCENT_UTILIZED);
            parametros.Parameters.Add("AREA_UTILIZED", Trxs_Codes.AREA_UTILIZED);
