...
{
    public class Reservation_Comment : IServiceJobs
    {
        private readonly ILogger<Reservation_Comment> _logger;
        ...
        public Reservation_Comment(ILogger<Reservation_Comment> logger, DapperContext context, IConfiguration config, Job_Exec Job_Exec)
        {
            _logger = logger;
            _context = context;
            _Job_Exec = Job_Exec;
        ...
        public void IntegrarJobAsync(string filePath, string id_job, string TIPO_AGENDAMENTO)
        {
            ...
            var List = _Job_Exec.Get_List<Mdl_Reservation_Comment>(filePath, LoggBuilder);
                    ...
                    foreach (var trial in List)
                    {
                        ...
                        try
                        {
                            var _where = $"RESORT = '{trial.RESORT}' and " +
                                        $"RESV_COMMENT_ID = '{trial.RESV_COMMENT_ID}' and " +
                                        $"RESV_NAME_ID = '{trial.RESV_NAME_ID}'";

                            var query = $"select RNA_UPDATEDATE from {TIPO_AGENDAMENTO} \n where {_where}";

                            var result = command.Connection.Query<Mdl_Reservation_Comment>(query);
        ...
        private OracleCommand MountParametros(OracleCommand parametros, Mdl_Reservation_Comment Trxs_Codes)
        {
            parametros.Parameters.Add("RESORT", Trxs_Codes.RESORT);
            parametros.Parameters.Add("RESV_COMMENT_ID", Trxs_Codes.RESV_COMMENT_ID);
            ...
            parametros.Parameters.Add("COMMENT_TYPE", Trxs_Codes.COMMENT_TYPE);
            ...
            parametros.Parameters.Add("NOTE_CODE", Trxs_Codes.NOTE_CODE);
            parametros.Parameters.Add("NOTE_RESORT", Trxs_Codes.NOTE_RESORT);
            parametros.Parameters.Add("INTERNAL_YN", Trxs_Codes.INTERNAL_YN);
            parametros.Parameters.Add("EXTERNAL_COMMENT_ID", Trxs_Codes.EXTERNAL_COMMENT_ID);
            parametros.Parameters.Add("USER_MODIFIABLE_YN", Trxs_Codes.USER_MODIFIABLE_YN);
            parametros.Parameters.Add("NOTE_TITLE", Trxs_Codes.NOTE_TITLE);
