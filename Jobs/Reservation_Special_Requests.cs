...
{
    public class Reservation_Special_Requests : IServiceJobs
    {
        private readonly ILogger<Reservation_Special_Requests> _logger;
        ...
        public Reservation_Special_Requests(ILogger<Reservation_Special_Requests> logger, DapperContext context, IConfiguration config, Job_Exec Job_Exec)
        {
            _logger = logger;
            _context = context;
            _Job_Exec = Job_Exec;
        ...
        public void IntegrarJobAsync(string filePath, string id_job, string TIPO_AGENDAMENTO)
        {
            ...
            var List = _Job_Exec.Get_List<Mdl_Reservation_Special_Requests>(filePath, LoggBuilder);
                    ...
                    foreach (var trial in List)
                    {
                        ...
                        try
                        {
                            var _where = $"RESORT = '{trial.RESORT}' and " +
                                        ...
                                        $"SPECIAL_REQUEST_ID = '{trial.SPECIAL_REQUEST_ID}'";

                            var query = $"select RNA_UPDATEDATE from {TIPO_AGENDAMENTO} \n where {_where}";

                            var result = command.Connection.Query<Mdl_Reservation_Special_Requests>(query);
        ...
        private OracleCommand MountParametros(OracleCommand parametros, Mdl_Reservation_Special_Requests Trxs_Codes)
        {
            parametros.Parameters.Add("RESORT", Trxs_Codes.RESORT);
            parametros.Parameters.Add("RESV_NAME_ID", Trxs_Codes.RESV_NAME_ID);
            parametros.Parameters.Add("SPECIAL_REQUEST_ID", Trxs_Codes.SPECIAL_REQUEST_ID);
            ...
            parametros.Parameters.Add("PRE_ARRIVAL_DT", Trxs_Codes.PRE_ARRIVAL_DT.ToString());
            parametros.Parameters.Add("EXTERNAL_SPECIAL_ID", Trxs_Codes.EXTERNAL_SPECIAL_ID);
            parametros.Parameters.Add("BASED_ON_RULE", Trxs_Codes.BASED_ON_RULE);
